...
public class MinimalPlayerMovement : MonoBehaviour {
	...
	void Update () {
        ...
        output += thisFramesInput.ToString();
        //edit to be relative to default rotation, not current view
        thisFramesInput = transform.rotation * thisFramesInput;
        
        
        //rotation could have rotated a 2d plane input into z component, dont want walking input to allow one to move up or down, so setting that back to 0
        thisFramesInput.y = 0;
        output += thisFramesInput.ToString();
        ...
        if(thisFramesInput.y != 0)
            Debug.Log(thisFramesInput.ToString());
        ...
        if(thisFramesInput.magnitude > 0)
        //Debug.Log(output);
        output = "";
	}
}
